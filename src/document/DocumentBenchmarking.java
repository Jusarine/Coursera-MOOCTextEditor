...
public class DocumentBenchmarking {
	...
	public static void main(String [] args) {
		...
		for (int numToCheck = start; numToCheck < numSteps*increment + start; numToCheck += increment)
		{
			...
			long firstTimeBd = System.nanoTime();
			long lastTimeBd = 0;
			for (int i = 0; i < trials; i++){
				BasicDocument bd = new BasicDocument(str);
				bd.getFleschScore();
				if (i == 3) lastTimeBd = System.nanoTime();
			}
			double timeBd = (double)(lastTimeBd - firstTimeBd)/1000000000;

			long firstTimeEd = System.nanoTime();
			long lastTimeEd = 0;
			for (int i = 0; i < trials; i++){
				EfficientDocument ed = new EfficientDocument(str);
				ed.getFleschScore();
				if (i == 5) lastTimeEd = System.nanoTime();
			}
			double timeEd = (double)(lastTimeEd - firstTimeEd)/1000000000;

			System.out.println("NumToCheck: " + numToCheck + '\t' + "Step3Bd: " +  timeBd + '\t' + "Step5Ed: " + timeEd);
